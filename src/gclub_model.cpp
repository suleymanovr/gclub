...
inline void
gclub::print_event(const string &time, const unsigned &id, const string &body)
{ cout << time << ' ' << id << ' ' << body << endl; }

inline void
gclub::split_body(const string &body, string &name, uint32_t &dskp_id)
{
...
void
gclub::ev12_client_landed(time_hh_mm &curr_time, client &land_client, 
...
void
gclub::ev11_client_leave(const string &time, const string &client_name)
{
...
void
gclub::ev13_error(const string &time, const string &error)
{
	cout << time << ' ' << "13 " << error << endl;
}

void
gclub::ev4_client_leave(event &ev, admin &adm)
{
...
void
gclub::ev3_client_wait(event &ev, admin &adm)
{
	time_hh_mm curr_time = ev.get_time();
	string     client_name{ev.get_body()};
	string     str_time   {curr_time.get_string_time()};
...
void
gclub::ev2_client_landed(event &ev, admin &adm)
{
	time_hh_mm curr_time = ev.get_time();
...
void 
gclub::ev1_client_came(event &ev, admin &adm)
{
	...
	if (curr_time.get_hh() <= start.get_hh() &&
		...
		||
		curr_time.get_hh() >= fin.get_hh() 	 &&
		curr_time.get_mm() >  fin.get_mm()) 
	{
...
void 
gclub::handle_event(event &new_event, admin &adm)
{
...
void
gclub::end_of_the_day(admin &adm)
{
