...
inline void 
perror_in_line(string &line, uint64_t lnum, const char* err)
{
	cout << "Error (line " << lnum << "): " << '"' << line << '"' << ". " 
														   << err << endl;
}
 ...
 */
inline int 
get_line(ifstream &file, string &line)
{
	int ret = 1;
	getline(file, line);
...
get_word(const string &ln, string &wrd, string::size_type pos0, 
...
*/
bool
parse_event_line(string &line, uint64_t lnum, gclub::event &new_event, 
 ...
 */
int 
parse_test_file(char *fname, struct gclub::input_params &test_data)
{
	string 	 	line, word;
	uint64_t 	lnum {1};
	std::size_t nconv{0};
	...
	if (file.is_open()){
		...
		for (unsigned i = 0; i < sizeof(delim_pat); i++) {
			if ((pos0 = get_word(line, word, pos0, delim_pat[i]))
					  == string::npos){
				perror_in_line(line, lnum, "Unable to get a word.");
				return 2;
			}
		...
		while ((pres = get_line(file, line)) == 0) {
			if (!parse_event_line(line, lnum, new_event, 
						          reftime, test_data.max_desktops))
			{
